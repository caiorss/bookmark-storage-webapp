...
import { Dialog2_Prompt, Dialog_YesNo, DialogForm, Dialog_Notify, Dialog_Datalist_Prompt } from "./dialogs.js";
...
dom.event_onContentLoaded(() => {
    ...
    set_theme(theme);
    if (theme == "dark_mode")
        theme_selection_box.selectedIndex = 0;
    if (theme == "light_mode")
        theme_selection_box.selectedIndex = 1;
    /*
...
export function item_quick_rename(item_id, old_item_title) {
    return __awaiter(this, void 0, void 0, function* () {
        let new_item_title = yield Dialog2_Prompt.prompt("Change item title:", "", old_item_title);
        ...
        let resp = yield tsutils.ajax_request(tsutils.HttpMethod.HTTP_PATCH, `/api2/items/${item_id}`, token, payload);
        // .ajax_request('/api2/items/' + item_id, token, "PATCH", payload)
        let data = yield resp.json();
        if (resp.is_status_success()) {
            let r = yield Dialog_Notify.notify("OK", "Item renamed Ok.", 1500);
            dom.page_refresh();
        }
        else {
...
export function item_set_starred(checkbox) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let resp = yield tsutils.ajax_request(HttpMethod.HTTP_PATCH, `/api2/items/${item_id}`, token, payload);
        if (resp.is_status_success()) {
            yield Dialog_Notify.notify("OK", "Toggle starred settings Ok.", 500);
            dom.page_refresh();
        }
        else {
...
export function api_item_print(item_id) {
    return __awaiter(this, void 0, void 0, function* () {
        var token = window["generated_token"];
        let item = yield tsutils.ajax_get("/api2/items/" + item_id, token);
...
function tag_create() {
    return __awaiter(this, void 0, void 0, function* () {
        let tag_name = yield Dialog2_Prompt.prompt("Enter new tag name:");
        if (tag_name == "")
            return;
        let token = window["generated_token"];
        let payload = { name: tag_name, description: "" };
        let resp = yield tsutils.ajax_request(HttpMethod.HTTP_POST, "/api/tags", token, payload);
        if (resp["result"] == "OK") {
            yield Dialog_Notify.notify_ok(resp["message"]);
            dom.page_refresh();
        }
        else {
            yield Dialog_Notify.notify_error(resp["message"]);
        }
    });
}
export function tag_add(item_id) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let token = window["generated_token"];
        let resp_ = yield tsutils.ajax_get("/api/tags", token);
        // console.log(all_tags);
        let data = yield resp_.json();
        for (let n in data) {
            let row = data[n];
            console.log(" row = ", row);
        ...
        let answer = yield dlg.prompt_selected();
        ...
        if (answer["key"] == null) {
            ...
            resp = yield tsutils.ajax_request(HttpMethod.HTTP_PUT, "/api/tags", token, payload);
        ...
        if (resp.is_status_success()) {
            yield Dialog_Notify.notify_ok("Tag added successfully. Ok.", 500);
            dom.page_refresh();
        }
        else {
            yield Dialog_Notify.notify_error(resp["message"], 500);
        }
    });
}
/** Delete tag from bookmark */
export function tag_delete_from_item(tag_id, bookmark_id) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        console.log(" Payload = ", payload);
        let token = window["generated_token"];
        let resp = yield tsutils.ajax_request(HttpMethod.HTTP_PUT, "/api/tags", token, payload);
        if (resp.is_status_success()) {
            yield Dialog_Notify.notify_ok("Tag removed from item. Ok.", 1000);
            dom.page_refresh();
        }
        else {
            yield Dialog_Notify.notify_error(" [FAILURE] " + resp["message"], 1000);
        }
    });
}
export function tag_delete(tag_name, tag_id) {
    return __awaiter(this, void 0, void 0, function* () {
        let answer = yield Dialog_YesNo.prompt("Delete tag.", `Are you sure you want to delete this tag: '${tag_name}'. 
                   Note: This action is irreversible. `);
        if (!answer) {
            return;
        }
        let resp = yield tsutils.ajax_request(HttpMethod.HTTP_PUT, "/api/tags", window["generated_token"], {
            "tag_name": tag_name,
            "tag_id": tag_id,
            "action": "delete_tag"
        });
...
export function tag_update(tag_name, tag_id, tag_desc) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let sender = yield dialog.onConfirm();
        let resp = yield tsutils.ajax_request(HttpMethod.HTTP_PUT, "/api/tags", window["generated_token"], {
            "action": "update_tag",
...
export function tag_filter_window() {
    return __awaiter(this, void 0, void 0, function* () {
        let dlg = new Dialog_Datalist_Prompt();
        dlg.setTitle("Select a tag");
        // dlg.setInputText(last_input.get());
        // Returns a list of tags [ { id: "tag id", name: "name", description: "Tag description"} ]
        let token = window["generated_token"];
        let resp = yield tsutils.ajax_get("/api/tags", token);
        let tags = yield resp.json();
        for (let n in tags) {
            let row = tags[n];
            // console.log(" row = ", row);
            // console.log(` name = ${row[name]} - id = ${row["id"]}`)
...
export function api_item_add(crfs_token) {
    return __awaiter(this, void 0, void 0, function* () {
        let url = yield Dialog2_Prompt.prompt("Enter the new URL to be added", "");
        ...
        if (query_params.get("filter") == "collection") {
            ...
            var token = window["generated_token"];
            let res = yield tsutils.ajax_post("/api/collections/add_item", token, payload1);
            if (res.is_status_success()) {
                let r = yield Dialog_Notify.notify("INFORMATION", "Bookmark added successfuly.", 2000);
                dom.page_refresh();
            }
            else if (res.is_domain_error()) {
                let msg = yield res.json();
        ...
        let payload = { url: url, starred: starred };
        console.log(" [TRACE] Payload = ", payload);
        let res = yield tsutils.ajax_post("/api2/items", crfs_token, payload);
...
export function item_delete(flag, item_id, item_title) {
    return __awaiter(this, void 0, void 0, function* () {
        let dialog_title = flag ? "Permanently delete item (Irreversible)." : "Delete item (move to trash)";
        let response = yield Dialog_YesNo.prompt(dialog_title, "Are you sure you want to delete item: " + item_title);
        if (!response)
            return;
        let mode = flag ? "hard" : "soft";
        let payload = { id: item_id, mode: mode };
        let token = window["generated_token"];
        let resp = yield tsutils.ajax_request(HttpMethod.HTTP_DELETE, "/api/items", token, payload);
        if (resp.is_status_success()) {
            let r = yield Dialog_Notify.notify("OK", "Item Deleted Ok.", 500);
            dom.page_refresh();
        }
        else {
...
export function item_upload_file() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        console.log(form);
        var token = window["generated_token"];
        let res = yield fetch("/api/item_upload", {
            method: 'POST',
            headers: {
        ...
        if (res.status == 200 || res.status == 201) {
            ...
            yield Dialog_Notify.notify("Information", "Upload successful.", 1500);
            dom.page_refresh();
        }
        else {
            yield Dialog_Notify.notify("Error", "Upload failure.", 1500);
        }
        // dom.page_refresh();
    });
}
...
export function item_snapshot(item_id) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let resp = yield tsutils.ajax_request(HttpMethod.HTTP_PUT, "/api/items", token, payload);
        if (resp.is_status_success()) {
            yield Dialog_Notify.notify_ok("Download successful Ok.");
            dlg.close();
            dom.page_refresh();
        }
        else {
...
export function item_snapshot_delete(item_id) {
    return __awaiter(this, void 0, void 0, function* () {
        let answer = yield Dialog_YesNo.prompt("Delete snapshot file.", `Are you sure you want to delete this snapshot file:`);
        if (!answer) {
            return;
        }
        let token = window["generated_token"];
...
export function search_bookmarks() {
...
export function clear_entry_field(dom_element_id) {
    let field = document.querySelector(dom_element_id);
    console.assert(field != null, `Cannot find entry field to be cleared`);
    field.value = "";
    field.focus();
}
export function related_item_add(item_id) {
    return __awaiter(this, void 0, void 0, function* () {
        let related_id = yield Dialog2_Prompt.prompt("Enter related item id:", "");
        let token = window["generated_token"];
        let payload = { item_id: item_id, related_ids: [related_id] };
        let res = yield tsutils.ajax_post("/api/related", token, payload);
        console.log(res);
        dom.page_refresh();
    });
}
...
export function toggle_items_table_info(table_info_id) {
    // alert("Button toggle clicked ok.");
    let obs = document.querySelector(table_info_id);
    if (obs == null) {
        alert(`Error: object #${table_info_id} not found. `);
    }
    // console.assert(obs, "Table info supposed not null.");
    dom.toggle(obs);
}
export function toggle_action_menu(actionID) {
    var elem = document.querySelector(actionID);
    dom.toggle(elem);
}
export function open_url_newtab(url) {
    dom.url_newtab(url);
}
...
function collection_create_new() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let sender = yield dialog_collection_edit.onConfirm();
        ...
        let res = yield tsutils.ajax_request(HttpMethod.HTTP_POST, "/api/collections", window["generated_token"], {
            title: title,
            description: desc
        });
        if (res["result"] == "OK") {
            let r = yield Dialog_Notify.notify("Information", "Collection created. Ok.", 500);
            dom.page_refresh();
        }
        else {
...
function collection_edit(collection_id, collection_title) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        //let desc  = sender.get_widget("desc").value;
        console.log(" [INFO] Creating collection with title = ", title);
        let res = yield tsutils.ajax_request(HttpMethod.HTTP_PUT, "/api/collections", window["generated_token"], {
            id: collection_id,
            title: title
            //, description: desc 
        });
...
function collection_delete(collection_id, collection_title) {
    return __awaiter(this, void 0, void 0, function* () {
        let answer = yield Dialog_YesNo.prompt("Delete collection.", `Are you sure you want to delete the collection: '${collection_title}' `);
        if (!answer) {
            return;
        }
        let resp = yield tsutils.ajax_request(HttpMethod.HTTP_DELETE, "/api/collections", window["generated_token"], { "collection_id": collection_id });
        if (resp.is_status_success()) {
